      ...
      curr match {
        case FsDirectory(_, children) =>
          children().foldLeft(acc) {
            ...
            case (results, dir2: FsDirectory) => (dfs(dir2, results) ++ Seq(curr).filter(filter).flatMap(processAndReport(_))).distinct
            case (results, f: FsFile) if results.contains(f) => results
            case (results, f: FsFile) => dfs(f, results)
          }
        case f @ FsFile(_) => (acc ++ Seq(f).filter(filter).flatMap(processAndReport(_))).distinct
      }
    }
